...
# Active Sessions Core Plugin for tmux-session-manager
# Manages currently active tmux sessions (excluding scratch)

plugin_meta() {
  echo "name:active"
  echo "description:Currently active tmux sessions"
  echo "depends:tmux"
  echo "install_cmd:brew install tmux"
  echo "icon:●"
  echo "icon_color:GREEN"
  echo "sort_priority:10"
}
...
plugin_resolve() {
  ...
  if tmux has-session -t "$session_name" 2>/dev/null; then
    ...
    # Basic session info without overriding other plugin types
