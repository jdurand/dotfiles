...
SketchyBar.exec("killall network_load >/dev/null; $CONFIG_DIR/helpers/event_providers/network_load/bin/network_load en0 network_update 2.0")

local popup_width = 250

local wifi_up = SketchyBar.add("item", "widgets.wifi1", {
  position = "right",
  padding_left = -5,
  width = 0,
  icon = {
...
local wifi_down = SketchyBar.add("item", "widgets.wifi2", {
  position = "right",
  padding_left = -5,
  icon = {
    padding_right = 0,
...
local wifi = SketchyBar.add("item", "widgets.wifi.padding", {
  position = "right",
  label = { drawing = false },
})
...
local ssid = SketchyBar.add("item", {
...
local hostname = SketchyBar.add("item", {
  position = "popup." .. wifi_bracket.name,
  icon = {
    align = "left",
    string = "Hostname:",
...
local mask = SketchyBar.add("item", {
  position = "popup." .. wifi_bracket.name,
  icon = {
    align = "left",
    string = "Subnet mask:",
...
local router = SketchyBar.add("item", {
  position = "popup." .. wifi_bracket.name,
  icon = {
    align = "left",
    string = "Router:",
...
local function toggle_details()
  ...
  if should_draw then
    ...
    SketchyBar.exec("networksetup -getinfo Wi-Fi | awk -F 'Subnet mask: ' '/^Subnet mask: / {print $2}'", function(result)
...
local function copy_label_to_clipboard(env)
  ...
  SketchyBar.set(env.NAME, { label = { string = icons.clipboard, align="center" } })
